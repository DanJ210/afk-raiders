...
When a shield blocks damage, the comms feed should include a follow-up line that shows the split between shield charge lost and HP damage landed. That keeps the log readable while still reflecting the shield math.
...
- Damage events should remain source-first: the event text announces the encounter, then a shield summary line can explain how much damage was absorbed.
