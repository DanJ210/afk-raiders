...
- **Balance contract:** Danger Level is the main survival lever. A starter Raider with no meaningful skill progress should survive Low often enough to keep the comedy loop moving, but should die frequently in Medium and especially High when left alone. Medium and High add small per-tick ambient downed pressure during RAIDING so dangerous conditions matter even before greed unlocks the scariest robot events. High danger is tuned as a Handler-intervention space: manual bandages, shield rechargers, and CALL EXTRACT are the intended survival path, not passive Raider Level scaling. Raider Level must not add HP, raw damage, shield strength, or major extraction safety; skill modifiers stay small enough that max skills still leave High danger harsher than Medium.
