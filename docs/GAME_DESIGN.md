...
4. **The Greed Check™ (signature mechanic):** Every tick in RAIDING, the Raider rolls to extract *or* keep looting. **Greed is event-driven:** it rises when triggered by specific raiding events (e.g., "spotted shiny loot in the distance") and is modified by Handler actions (Calm lowers it, Pressure raises it). Higher greed no longer directly lowers extraction chance or adds death pressure; instead it nudges rare loot odds during the current raid and decays into major-condition momentum for the next deployment. Pure dramatic tension, zero input required — the Handler can only nudge via Signal actions.
...
- On every return to HUB, raid pressure cools down: greed decays instead of resetting to 0, forced-extract is cleared, and pending handler actions are consumed/cleared before the next raid.
