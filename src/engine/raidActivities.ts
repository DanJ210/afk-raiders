...
import type { ActivityLogEvent, ActiveRaidActivity, BackpackItem, GameState, HealingItem, LootItem, RaidActivityDefinition, RobotEntry, RobotLootItem, ShieldRechargerItem, StartRaidActivityEffect } from './types.js'
...
export function advanceRaidActivity(state: GameState, rng: RNG, now: number): AdvanceRaidActivityResult {
  ...
  const raiderDamage = rng.int(activity.raiderDamageMin ?? DEFAULT_RAIDER_WEAPON.damageMin, activity.raiderDamageMax ?? DEFAULT_RAIDER_WEAPON.damageMax)
  ...
  if (nextTicksRemaining <= 0) {
    const text = fillActivityText(definition.text.failed, {
      robot_name: robot.name,
      weapon_name: activity.weaponName ?? DEFAULT_RAIDER_WEAPON.name,
      raider_damage: raiderDamage,
    ...
    return {
      state: {
        ...state,
        raider: shieldDamage.raider,
        raid: { ...shieldDamage.raid, activeRaidActivity: null },
      },
      activityEvents: [activityLogEvent(activity, 'failed', state.tick, now, text)],
      blocking: true,
      robotSurvivedId: robot.id,
    }
  }
  ...
  return {
    state: {
      ...state,
      raider: shieldDamage.raider,
      raid: { ...shieldDamage.raid, activeRaidActivity: nextActivity },
    },
...
function advanceSearchActivity(
  ...
  if (nextTicksRemaining <= 0) {
    ...
    const loot = lootItemToBackpackItem(rng.weightedPick(lootTable))
    const nextRaid = addBackpackItem({ ...state.raid, activeRaidActivity: null }, loot)
    const text = fillActivityText(definition.text.completed, {
      loot_name: loot.name,
      ticks_remaining: 0,
