    ...
    <div class="grid gap-2">
      ...
      <div v-if="viewModel.extractZoneRows.value.length > 0 || viewModel.deathZoneRows.value.length > 0" class="min-w-0">
        ...
        <ul class="list-none m-0 p-0 flex flex-col gap-0.5">
          <li v-for="[zoneId, count] in viewModel.extractZoneRows.value" :key="`extract-zone-${zoneId}`" class="font-mono text-[0.72rem] text-text wrap-anywhere">
            ✅ {{ viewModel.zoneLabel(zoneId) }}: {{ count }}
          </li>
          <li v-for="[zoneId, count] in viewModel.deathZoneRows.value" :key="`death-zone-${zoneId}`" class="font-mono text-[0.72rem] text-text wrap-anywhere">
            💀 {{ viewModel.zoneLabel(zoneId) }}: {{ count }}
          </li>
        </ul>
      </div>
      ...
      <div v-if="viewModel.extractZoneTimeRows.value.length > 0 || viewModel.deathZoneTimeRows.value.length > 0" class="min-w-0">
        ...
        <ul class="list-none m-0 p-0 flex flex-col gap-0.5">
          <li v-for="[key, count] in viewModel.extractZoneTimeRows.value" :key="`extract-zone-time-${key}`" class="font-mono text-[0.72rem] text-text wrap-anywhere">
            ✅ {{ viewModel.zoneLabel(viewModel.parseZoneDangerLevelKey(key).zoneId) }} ({{ viewModel.parseZoneDangerLevelKey(key).dangerLevel }}): {{ count }}
          </li>
          <li v-for="[key, count] in viewModel.deathZoneTimeRows.value" :key="`death-zone-time-${key}`" class="font-mono text-[0.72rem] text-text wrap-anywhere">
            💀 {{ viewModel.zoneLabel(viewModel.parseZoneDangerLevelKey(key).zoneId) }} ({{ viewModel.parseZoneDangerLevelKey(key).dangerLevel }}): {{ count }}
          </li>
        </ul>
      </div>
      ...
      <div v-if="viewModel.robotRows.value.length > 0" class="min-w-0">
        ...
        <ul class="list-none m-0 p-0 flex flex-col gap-0.5">
          <li v-for="[robotId, count] in viewModel.robotRows.value" :key="`robot-${robotId}`" class="font-mono text-[0.72rem] text-text wrap-anywhere">
      ...
      <div v-if="stats.healingItemsUsed.total > 0" class="min-w-0">
        ...
        <p class="m-0 font-mono text-[0.72rem] text-muted wrap-anywhere">Total uses: {{ stats.healingItemsUsed.total }}</p>
        <ul class="list-none m-0 p-0 flex flex-col gap-0.5 mt-1">
          <li v-for="[itemId, count] in viewModel.healingRows.value" :key="`healing-${itemId}`" class="font-mono text-[0.72rem] text-text wrap-anywhere">
