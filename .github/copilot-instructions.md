...
- These rarity biases (danger-level profile, mood, greed) apply to both diary-event loot and SEARCH activity loot rolls.
- SEARCH activity `lootTableId` accepts a single table id or an array of table ids; arrays are merged into one weighted pick pool.
...
- Raid-timer expiry with no extraction in progress transitions straight to `KNOCKED_OUT` (no DOWNED). The timer-zero DOWNED race exists only when extraction is already active, fires once per raid (`RaidState.raidTimeoutDownedStarted`), and never re-downs a revived raider.
- Reviving (Handler REVIVE or a revive med) clears `RaidState.downed` and immediately emits a `completed` entry for the DOWNED activity thread so the active-thread log closes cleanly.
...
### Comms Priority
Every emitted `LogEvent` carries a `commsPriority` field (`CommsPriority`: `ambient` | `priority` | `activity`). Ambient chatter is suppressed on ticks where priority comms are already queued; at most one ambient line lands per tick. Event JSON templates (and raid activity definitions) may declare `commsPriority` directly; the resolver honors template values first and falls back to id-based classification for unannotated content. New content should declare the field explicitly rather than relying on the fallback.
...
| 2 | `moderate` | Tattletale drone (menace 3), Passive-Aggressor Drone (menace 4), Seeker of Validation (menace 5), Harvester of Mild Annoyances (menace 4), Receipt Printer of Doom (menace 5) |
| 3 | `dangerous` | Walker Texas Malfunction (menace 6), Enforcer of Minor Inconveniences (menace 6), Apology Turret (menace 6) |
| 4 | `nasty` | Bomber Who Misreads the Room (menace 7) |
| 5 (deadliest) | `deadly` | Roomba Prime (menace 8), Crusher of Dreams (menace 8), Sniper of Poor Decisions (menace 9), Tank of Overcompensation (menace 10), The Drama Queen (boss, menace 10) |

Boss robots (`isBoss: true`, currently The Drama Queen) are excluded from generic robot pools unless the pool sets `includeBosses: true`.
...
Per-robot voice lives in `robots.json` `flavorLines`. Ambient overlay events gated to an active `ROBOT_ENCOUNTER` can use the context-aware slots `{robot_name}` and `{robot_flavor}` (resolved from the current encounter's robot); these slots are only valid on events that require an active robot encounter, and content tests enforce this. To add robot personality, extend `flavorLines` — do not write per-robot event copies.
