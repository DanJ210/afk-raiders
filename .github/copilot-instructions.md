...
- `LogEvent.conditions` stores active RAIDING conditions as an ordered array. Use `logConditionsForRaid()` from `src/engine/log.ts` for all engine log emission so EXTRACTING+DOWNED overlap is preserved consistently (`['EXTRACTING', 'DOWNED']`). The comms UI renders condition icons from this metadata before falling back to phase icons.
